...
fn benchmarks(c: &mut Criterion) {
    let bench_crc32 = Fun::new("CRC 32", |b, _i| b.iter(|| crc32::make_table(crc32::IEEE, true)));

    let table = crc32::make_table(crc32::IEEE, true);
    let bytes = Box::new([0u8; 100_000]);
    let bench_crc32_mb = Fun::new("CRC 32 MBs", move |b, _i| b.iter(|| crc32::update(0, &table, &*bytes, true)));

    let bench_crc64 = Fun::new("CRC 64", |b, _i|  b.iter(|| crc64::make_table(crc64::ECMA)));

    let table2 = crc64::make_table(crc64::ECMA);
    let bytes2 = Box::new([0u8; 100_000]);
    let bench_crc64_mb = Fun::new("CRC 64 MBs", move |b, _i| b.iter(|| crc64::update(0, &table2, &*bytes2)));
