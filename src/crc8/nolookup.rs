...
impl Crc<u8, Table<0>> {
    ...
    pub const fn digest(&self) -> Digest<u8, Table<0>> {
    ...
    pub const fn digest_with_initial(&self, initial: u8) -> Digest<u8, Table<0>> {
        let value = init(self.algorithm, initial);
        Digest::new(self, value)
    }
}

impl<'a> Digest<'a, u8, Table<0>> {
    const fn new(crc: &'a Crc<u8, Table<0>>, value: u8) -> Self {
        Digest { crc, value }
    }
