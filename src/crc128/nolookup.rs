...
impl Crc<u128, Table<0>> {
    ...
    pub const fn digest(&self) -> Digest<u128, Table<0>> {
    ...
    pub const fn digest_with_initial(&self, initial: u128) -> Digest<u128, Table<0>> {
        let value = init(self.algorithm, initial);
        Digest::new(self, value)
    }
}

impl<'a> Digest<'a, u128, Table<0>> {
    const fn new(crc: &'a Crc<u128, Table<0>>, value: u128) -> Self {
        Digest { crc, value }
    }
