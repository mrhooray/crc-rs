...
impl Crc<u16, Table<0>> {
    ...
    pub const fn digest(&self) -> Digest<u16, Table<0>> {
    ...
    pub const fn digest_with_initial(&self, initial: u16) -> Digest<u16, Table<0>> {
        let value = init(self.algorithm, initial);
        Digest::new(self, value)
    }
}

impl<'a> Digest<'a, u16, Table<0>> {
    const fn new(crc: &'a Crc<u16, Table<0>>, value: u16) -> Self {
        Digest { crc, value }
    }
