...
impl Crc<u64, Table<0>> {
    ...
    pub const fn digest(&self) -> Digest<u64, Table<0>> {
    ...
    pub const fn digest_with_initial(&self, initial: u64) -> Digest<u64, Table<0>> {
        let value = init(self.algorithm, initial);
        Digest::new(self, value)
    }
}

impl<'a> Digest<'a, u64, Table<0>> {
    const fn new(crc: &'a Crc<u64, Table<0>>, value: u64) -> Self {
        Digest { crc, value }
    }
